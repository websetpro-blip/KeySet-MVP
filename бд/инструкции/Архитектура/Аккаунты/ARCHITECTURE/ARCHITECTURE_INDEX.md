...
**[FEATURE_READINESS.md](./FEATURE_READINESS.md)** — Готовность функционала
   - Процент готовности каждого модуля
   - Детальный разбор реализованных функций
...
### 🔧 Рефакторинг и Консолидация

17. **[17_MODULE_CONSOLIDATION.md](./17_MODULE_CONSOLIDATION.md)** — Консолидация модулей
    - Перенос модулей из `keyset/` в плоскую структуру `core/`, `services/`, `utils/`
    - Обновление импортов с относительных на абсолютные
    - Оптимизация PyInstaller сборки (91690 → 413 файлов)
    - Автоматическое определение пути к БД
    - Новая структура проекта и best practices
...
1. **Начните с [FEATURE_READINESS.md](./FEATURE_READINESS.md)** — чтобы понять текущее состояние проекта
2. **Изучите [11_DATA_FLOW.md](./11_DATA_FLOW.md)** — чтобы понять общую картину
3. **Прочитайте [01_DATABASE.md](./01_DATABASE.md)** — структуру данных
4. **Изучите [06_PARSING.md](./06_PARSING.md)** — ядро системы
5. **Погрузитесь в нужный модуль** — по необходимости (08-09 для frontend, 10-11 для интеграции)
