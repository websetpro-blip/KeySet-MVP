...
function setupEventListeners() {
    ...
    // Новые функции
    const browserLauncherBtn = document.getElementById('browserLauncherBtn');
    const consistencyCheckerBtn = document.getElementById('consistencyCheckerBtn');
    
    if (browserLauncherBtn) {
        browserLauncherBtn.addEventListener('click', handleBrowserLaunch);
    }
    
    if (consistencyCheckerBtn) {
        consistencyCheckerBtn.addEventListener('click', handleConsistencyCheck);
    }
...
// Новые функции для кнопок в top-bar
function handleBrowserLaunch() {
    showToast('🚀 Запуск браузера...', 'info');
    
    // Симуляция запуска браузера
    setTimeout(() => {
        showToast('✅ Браузер успешно запущен!', 'success');
        console.log('Browser launch initiated');
    }, 2000);
}

function handleConsistencyCheck() {
    showToast('🔍 Проверка консистентности данных...', 'info');
    
    // Симуляция проверки консистентности
    setTimeout(() => {
        const accounts = document.querySelectorAll('#accountsTableBody tr');
        const issues = [];
        ...
        accounts.forEach((row, index) => {
            ...
            if (email && status && status.includes('❌')) {
                issues.push(`${email} - проблема с авторизацией`);
            }
        });
        
        if (issues.length > 0) {
            showToast(`⚠️ Найдено ${issues.length} проблем консистентности`, 'warning');
            console.log('Consistency issues found:', issues);
        } else {
            showToast('✅ Консистентность данных в порядке!', 'success');
