(function () {
  ...
  window.fetch = async (input, init = {}) => {
    ...
    const matched = routes.find((item) => request.startsWith(item.prefix));
    if (!matched) {
      return originalFetch(input, init);
    }
    ...
    try {
      const payload =
        init && typeof init.body === "string" && init.body.length
          ...
          : {};
      const data = await callPython(matched.action, payload);
  ...
  console.info("[KeySet] accounts bridge ready");
})();
