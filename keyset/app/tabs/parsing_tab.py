...
from ..dialogs.parsing_dialogs import StopwordsDialog, FilterDialog, DuplicatesDialog
...
class ParsingTab(QWidget):
    ...
    def _create_toolbar(self) -> QToolBar:
        ...
        self.btn_stopwords = QToolButton()
        self.btn_stopwords.setText("🚫 Стоп-слова")
        self.btn_stopwords.setToolButtonStyle(Qt.ToolButtonTextOnly)
        self.btn_stopwords.setToolTip("Минусация - удаление фраз со стоп-словами")

        self.btn_filters = QToolButton()
        self.btn_filters.setText("🔍 Фильтры")
        self.btn_filters.setToolButtonStyle(Qt.ToolButtonTextOnly)
        self.btn_filters.setToolTip("Фильтрация по частоте и количеству слов")

        self.btn_duplicates = QToolButton()
        self.btn_duplicates.setText("🔄 Дубли")
        self.btn_duplicates.setToolButtonStyle(Qt.ToolButtonTextOnly)
        self.btn_duplicates.setToolTip("Найти и удалить дубликаты")
        ...
        toolbar.addWidget(self.btn_stopwords)
        toolbar.addWidget(self.btn_filters)
    ...
    def _wire_signals(self):
        ...
        self.btn_stopwords.clicked.connect(self._on_stopwords_clicked)
        self.btn_filters.clicked.connect(self._on_filters_clicked)
        self.btn_duplicates.clicked.connect(self._on_duplicates_clicked)
    ...
    def _on_add_from_file(self) -> None:
        """Добавить фразы из текстового файла (TXT/CSV/XLSX)."""
    ...
    def _on_batch_collect_requested(self, phrases: List[str], settings: dict):
        """Обработка запроса пакетного сбора фраз из левой колонки Wordstat"""
        if not phrases:
        ...
        # Нормализуем настройки и сохраняем их
        ...
        self._append_log("=" * 70)
        self._append_log("🚀 Запуск парсинга левой колонки...")

        # СРАЗУ запускаем парсинг левой колонки
    ...
    def _run_left_column_parsing(self, phrases: List[str], profiles: List[dict], region_id: int, threshold: int):
        """Запустить парсинг левой колонки Wordstat"""
        from pathlib import Path
        ...
        if profile_path_str:
            ...
            profile_path = Path(".profiles") / profile_email

        proxy = profile.get("proxy")

        self._append_log(f"📂 Профиль: {profile_path}")
        if proxy:
            self._append_log(f"🌐 Прокси: {proxy}")
        ...
        parser = LeftColumnParser(
            ...
            region_id=region_id,
            min_shows=threshold,
            max_results=200,  # Топ-200 подсказок для каждой фразы (кликает "Показать ещё")
        ...
        try:
            ...
            try:
                loop = asyncio.get_running_loop()
                # Если loop уже запущен, используем run_until_complete через новый поток
                self._append_log("⚠️ Event loop уже запущен, создаю новый...")
                loop = asyncio.new_event_loop()
            except RuntimeError:
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
            ...
            # Запускаем парсинг
            self._append_log("⏳ Парсинг запущен...")
            ...
            for parent_phrase, suggestions in results.items():
                if not suggestions:
                    self._append_log(f"📂 '{parent_phrase}' → подсказок не найдено")
                    continue

                self._append_log(f"\n📂 '{parent_phrase}' → найдено {len(suggestions)} подсказок:")

                # Показываем первые 10 в логе
                ...
                if len(suggestions) > 10:
                    ...
                    # Добавляем остальные в таблицу (без логов)
                    for item in suggestions[10:]:
                        phrase = item["phrase"]
                        self._insert_phrase_row(phrase, status="OK", checked=False)
                        total_found += 1
            ...
            import traceback
            self._append_log(f"Подробности:\n{traceback.format_exc()}")
    ...
    # ============= НОВЫЕ ФУНКЦИИ ИЗ V5.0 =============

    def _on_stopwords_clicked(self):
        """Обработчик кнопки Стоп-слова"""
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            for row in range(self.keys_table.rowCount()):
                phrase_item = self.keys_table.item(row, 0)
                if not phrase_item:
                    continue
                phrase = phrase_item.text()
            ...
            mode_names = {
                ...
                'morphological': 'морфонезависимое'
            }
            self._append_log(f"🚫 Удалено фраз со стоп-словами ({mode_names.get(mode, mode)}): {removed_count}")

    def _on_filters_clicked(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            for row in range(self.keys_table.rowCount()):
                ...
                # Фильтр по частоте
                freq_item = self.keys_table.item(row, 1)
                if freq_item:
                    try:
                        freq = int(freq_item.text())
                        if not (filters['freq_min'] <= freq <= filters['freq_max']):
                            show_row = False
                    except ValueError:
                        pass
                ...
                phrase_item = self.keys_table.item(row, 0)
                if phrase_item and show_row:
                    words_count = len(phrase_item.text().split())
                    if not (filters['words_min'] <= words_count <= filters['words_max']):
                        show_row = False

                # Фильтр по тексту
                if filters['search_text'] and show_row:
                    if phrase_item and filters['search_text'].lower() not in phrase_item.text().lower():
                        show_row = False

                self.keys_table.setRowHidden(row, not show_row)
                if show_row:
                    visible_count += 1

            self._append_log(f"🔍 Фильтр применен: видно {visible_count} из {self.keys_table.rowCount()} фраз")

    def _on_duplicates_clicked(self):
        """Обработчик кнопки Дубли"""
        phrases_dict = {}
        duplicates = []
        ...
        for row in range(self.keys_table.rowCount()):
            ...
            if phrase in phrases_dict:
                ...
                phrases_dict[phrase].append(row)
            else:
                phrases_dict[phrase] = [row]

        if not duplicates:
            QMessageBox.information(self, "Дубли", "Дублей не найдено")
            return
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            self._append_log(f"🔄 Удалено дублей: {len(rows_to_remove)}")

    def add_table_checkboxes(self):
        """Добавить чекбоксы в первый столбец таблицы"""
